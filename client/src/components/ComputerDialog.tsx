...
export default function ComputerDialog() {
  ...
  const computerUrl = computerId ? useAppSelector((state) => state.computer.urls.get(computerId)) : null;
  const dispatch = useAppDispatch();
  ...
  return (
    <Backdrop>
      <Wrapper>
        ...
        {computerUrl ? (
          <ComputerWrapper>
            <iframe title="computer interface" src={computerUrl} />
          </ComputerWrapper>
        ) : (
          <div style={{ padding: '20px', textAlign: 'center' }}>
            <h2>Computer Interface</h2>
            <p>No content available for this computer.</p>
          </div>
        )}
      </Wrapper>
    </Backdrop>
